...
def saliency(model, input_ids, input_mask, pretrained, batch=0, correct=None, foil=None):
    ...
    if pretrained not in ['google/flan-t5-large', 't5-large']:
        input_ids = torch.tensor(input_ids, dtype=torch.long).to(model.device)
        input_mask = torch.tensor(input_mask, dtype=torch.long).to(model.device)
    else:
        input_ids = torch.tensor([input_ids], dtype=torch.long).to(model.device)
    ...
    if pretrained not in ['google/flan-t5-large', 't5-large']:
        ...
        A = model(input_ids, decoder_input_ids=input_ids, attention_mask=input_mask)
    ...
    if pretrained not in ['google/flan-t5-large', 't5-large']:
        ...
        if foil is not None and correct != foil:
            (A.logits[0][-1][correct]-A.logits[-1][foil]).backward()
        else:
